...
|<nobr>`%s/\([^ ]\+\)-\n\([^ ]\+\)/\r\1\2/g`</nobr>|Silbentrennungen sollen entfernt werden, und zwar so, dass vor dem getrennten Wort ein neuer Absatz eingefügt wird. Es wird nach Wörtern gesucht, die einen Trennstrich gefolgt von einem Absatz enthalten. Der Trennstrich wird entfernt und stattdessen wird vor dem Wort ein neuer Absatz eingefügt.|
